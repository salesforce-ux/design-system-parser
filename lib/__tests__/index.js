...
describe('parser', () => {
  ...
  describe('component', () => {
    describe('data tables', () => {
      ...
      it('adds the restrictee hierarchy tree', () => {
        ...
        expect(
          table.get('restrictees').first().get('restrictees').count()
        ).toEqual(10)
  ...
  describe('utility', () => {
    ...
    describe('with includeRestrictedModifiers option', () => {
      let parserWithOptions
      let bordersWithOptions
      beforeEach(() => {
        parserWithOptions = require('../../')(scss, { includeRestrictedModifiers: true })
        bordersWithOptions = parserWithOptions.utility('borders').getOrElse(null)
      })
      it('includes restricted modifiers when option is enabled', () => {
        // The borders utility should include any modifiers that are restricted to elements
        // matching the borders utility selector pattern
        expect(bordersWithOptions.get('restrictees').count()).toBeGreaterThanOrEqual(4)
      })
    })
  })
  ...
  describe('moduleComponent', () => {
    ...
    describe('zigzag', () => {
      ...
      it('adds the restrictee hierarchy tree', () => {
        ...
        expect(
          zigzag.get('restrictees').first().get('restrictees').count()
        ).toEqual(1)
      ...
      it('does not add itself as a restrictee', () => {
        expect(zigzag.get('restrictees').first().get('id')).toEqual('base')
      })
    })
  })
})
