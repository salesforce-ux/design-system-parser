...
module.exports = (comments, options = {}) => {
  ...
  const findAllByRestrict = (selector) =>
    fromNullable(INDEX.get(selector))
      ...
      .getOrElse(List())
      .concat(
        fancy.filter((comment) =>
          selectorMatches('restrict', selector, comment)
        )
      )
  ...
  const initialize = () => {
    ...
    // Debug: Check what comments are being processed
    console.log('DEBUG: Total comments:', comments.count())
    console.log('DEBUG: Comments after topLevel filter:', normalized.count())

    // Debug: Check if box modifiers are in the original comments
    const boxModifiersOriginal = comments.filter(comment =>
      comment.getIn(['annotations', 'selector']) &&
      comment.getIn(['annotations', 'selector']).includes('slds-box_')
    )
    console.log('DEBUG: Box modifiers in original comments:', boxModifiersOriginal.count())
    boxModifiersOriginal.forEach(modifier => {
      console.log('  -', modifier.getIn(['annotations', 'selector']), 'restrict:', modifier.getIn(['annotations', 'restrict']), 'topLevel:', topLevel(modifier))
    })

    // Debug: Check if box modifiers are in the normalized comments
    const boxModifiers = normalized.filter(comment =>
      comment.getIn(['annotations', 'selector']) &&
      comment.getIn(['annotations', 'selector']).includes('slds-box_')
    )
    console.log('DEBUG: Box modifiers in normalized:', boxModifiers.count())
    boxModifiers.forEach(modifier => {
      console.log('  -', modifier.getIn(['annotations', 'selector']), 'restrict:', modifier.getIn(['annotations', 'restrict']))
    })
    ...
    console.log('DEBUG: Plain comments count:', plain.count())
    console.log('DEBUG: Fancy comments count:', fancy.count())
    console.log('DEBUG: Utils comments count:', utils.count())
  ...
  console.log('DEBUG: INDEX keys:', INDEX.keySeq().toArray())
  console.log('DEBUG: INDEX for .slds-box:', INDEX.get('.slds-box') ? INDEX.get('.slds-box').count() : 'not found')
  ...
  return {
    ...
    findVariants,
    options,
    comments: comments // Store the original comments
  }
}
